...
UPDATE `sys_menu_mobile` SET `eval_bubble` = 'return BxDolXMLRPCMedia::_getMediaCount(''video'', ''{profile_id}'', ''{member_id}'');' WHERE `type` = '[db_prefix]' AND `page` = 'profile' AND `eval_bubble` = 'return BxDolXMLRPCMedia::_getMediaCount(''video'', ''{profile_id}'', ''{member_id}'')';


UPDATE `sys_modules` SET `version` = '1.0.8' WHERE `uri` = 'videos' AND `version` = '1.0.7';

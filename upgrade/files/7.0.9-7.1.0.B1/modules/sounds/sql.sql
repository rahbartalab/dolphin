...
UPDATE `sys_albums` SET `AllowAlbumView` = 8 WHERE `Caption` = @sHiddenAlbumName AND `Type` = 'bx_sounds';
...
SET @iKatID = (SELECT `ID` FROM `sys_options_cats` WHERE `name` = 'Sounds');
...
('[db_prefix]_view', 'Listen Sound', @iPCPOrder+1),
('[db_prefix]_home', 'Sounds Home', @iPCPOrder+2),
('[db_prefix]_rate', 'Sounds Rate', @iPCPOrder+3),
('[db_prefix]_album_view', 'Sound Album', @iPCPOrder+4),
('[db_prefix]_albums_owner', 'Profile Albums', @iPCPOrder+5);

DELETE FROM `sys_page_compose` WHERE `Page` IN ('index', 'profile', 'member') AND `Desc` IN ('Public Sounds', 'Sound Albums', 'Profile Sounds');
...
UPDATE `sys_menu_top` SET `Picture` = 'music' WHERE `Parent` = 0 AND `Type` = 'system' AND `Name` = 'SoundsUnit';
UPDATE `sys_menu_top` SET `Picture` = 'music' WHERE `Parent` = 0 AND `Type` = 'system' AND `Name` = 'SoundsAlbum';
UPDATE `sys_menu_top` SET `Link` = 'modules/?r=sounds/albums/browse/owner/{profileUsername}' WHERE `Parent` = 9 AND `Name` = 'Sounds';
UPDATE `sys_menu_top` SET `Link` = 'modules/?r=sounds/albums/my/main/|modules/?r=sounds/albums/my/add/|modules/?r=sounds/albums/my/manage/|modules/?r=sounds/albums/my/disapproved/' WHERE `Parent` = 4 AND `Name` = 'Sounds';

SET @iMenuFilesTop = (SELECT `ID` FROM `sys_menu_top` WHERE `Parent` = 0 AND `Type` = 'top' AND `Name` = 'Sounds');
UPDATE `sys_menu_top` SET `Picture` = 'music', `Icon` = 'music' WHERE `ID` = @iMenuFilesTop;
UPDATE `sys_menu_top` SET `Picture` = '' WHERE `Parent` = @iMenuFilesTop AND `Name` = 'SoundsHome';
...
DELETE FROM `sys_email_templates` WHERE `Name` IN ('t_bx_sounds_share', 't_bx_sounds_report', 't_sbs_bx_sounds_comments', 't_sbs_bx_sounds_rates');
...
('pmu', '[db_prefix]', 'modules/?r=sounds/browse/all', 'SELECT COUNT(`ID`) FROM `RayMp3Files` WHERE `Status`=''approved''', 'modules/?r=sounds/administration/disapproved', 'SELECT COUNT(`ID`) FROM `RayMp3Files` WHERE `Status`=''disapproved''', 'music', @iStatSiteOrder);


-- menu admin

UPDATE `sys_menu_admin` SET `icon` = 'music' WHERE `name` = '[db_prefix]';
...
DELETE `sys_localization_strings` FROM `sys_localization_strings`, `sys_localization_keys` WHERE `sys_localization_keys`.`ID` = `sys_localization_strings`.`IDKey` AND `sys_localization_keys`.`Key` IN('_bx_sounds_sbs_main','_bx_sounds_sbs_rates','_bx_sounds_wall_music');
DELETE FROM `sys_localization_keys` WHERE `Key` IN('_bx_sounds_sbs_main','_bx_sounds_sbs_rates','_bx_sounds_wall_music');



-- update module version

UPDATE `sys_modules` SET `version` = '1.1.0' WHERE `uri` = 'sounds' AND `version` = '1.0.9';

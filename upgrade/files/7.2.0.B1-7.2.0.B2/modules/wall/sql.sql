...
DELETE FROM `sys_page_compose` WHERE `Page` = 'wall';
DELETE FROM `sys_page_compose` WHERE `Caption` IN ('_wall_pc_post', '_wall_pc_view', '_wall_pc_view_account', '_wall_pc_view_index');
DELETE FROM `sys_page_compose` WHERE `Caption` IN ('_wall_pc_view_index_ol', '_wall_pc_post_index_tl', '_wall_pc_view_index_tl', '_wall_pc_post_profile_tl', '_wall_pc_view_profile_tl', '_wall_pc_view_account_tl');
...
SET @iCategoryId = (SELECT `ID` FROM `sys_options_cats` WHERE `name` = 'Timeline');
